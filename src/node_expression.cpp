...
void NodeExpression::Init(v8::Local<v8::Object> target) {
    ...
    constructor.Reset(Nan::GetFunction(tpl).ToLocalChecked()); // what is this doing?
...
type::Type parseType(v8::Local<v8::Object> type) {
    ...
    std::string kind(*Nan::Utf8String(v8kind));

    if (kind == "array") {
        type::Type itemType = parseType(
          Nan::To<v8::Object>(
            Nan::Get(type, Nan::New("itemType").ToLocalChecked()).ToLocalChecked()
          ).ToLocalChecked()
        );
...
void NodeExpression::Parse(const Nan::FunctionCallbackInfo<v8::Value>& info) {
    ...
    if (info.Length() > 1 && info[1]->IsObject()) {
        expected = parseType(Nan::To<v8::Object>(info[1]).ToLocalChecked());
    }
...
void NodeExpression::Evaluate(const Nan::FunctionCallbackInfo<v8::Value>& info) {
    ...
    v8::Local<v8::Value> v8zoom = Nan::Get(
      Nan::To<v8::Object>(info[0]).ToLocalChecked(),
      Nan::New("zoom").ToLocalChecked()
    ).ToLocalChecked();
    if (v8zoom->IsNumber()) zoom = Nan::To<double>(v8zoom).ToChecked();

    mbgl::optional<double> heatmapDensity;
    v8::Local<v8::Value> v8heatmapDensity = Nan::Get(
      Nan::To<v8::Object>(info[0]).ToLocalChecked(),
      Nan::New("heatmapDensity").ToLocalChecked()
    ).ToLocalChecked();
    if (v8heatmapDensity->IsNumber()) heatmapDensity = Nan::To<double>(v8heatmapDensity).ToChecked();
